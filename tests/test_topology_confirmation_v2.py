import unittest
import numpy as np
from scripts.run_topology_confirmation_v2 import assign_test, analysis
...
class ConfirmationTests(unittest.TestCase):
    ...
    def test_analysis_requires_all_pairs(self):
        with self.assertRaises(ValueError):analysis([.9]*4,[.5]*4)
    def test_null_and_direction(self):
        self.assertFalse(analysis([.5]*5,[.5]*5)['primary_pass'])
        x=np.arange(5)/100+.7;y=np.ones(5)*.5
        positive=analysis(x,y);negative=analysis(y,x)
        self.assertTrue(positive['primary_pass']);self.assertFalse(negative['primary_pass'])
        self.assertAlmostEqual(positive['sign_flip_p'],2/32)
