"""Isolated G2c engine: deterministic formal tasks, exact answers, fixed graph."""
import argparse
...
def train(spec_path,until):
    ...
    optimizer=torch.optim.AdamW(model.parameters(),lr=spec['lr'],weight_decay=.01,foreach=False)
    generator=torch.Generator().manual_seed(spec['sampling_seed']);records=data(spec['dataset'],'train')
    ...
    for step in range(done+1,until+1):
        ...
        if spec['objective']=='kd':
            ...
            loss=spec['alpha']*ce+(1-spec['alpha'])*kl
        optimizer.zero_grad(set_to_none=True);loss.backward();grad=nn.utils.clip_grad_norm_(model.parameters(),1.)
