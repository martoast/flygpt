...
CFG = Path('configs/topology_confirmation_v2.json')
...
def prepare_data(cfg):
    ...
    freeze(dest/'manifest.json', dict(version='topology-confirmation-v2', task='substitute', length=6,
...
def analysis(real, rewired):
    ...
    if real.shape != (5,) or rewired.shape != (5,) or not np.isfinite([real,rewired]).all():
        raise ValueError('Exactly five finite paired results required')
    gaps=real-rewired; mean=float(gaps.mean()); sd=float(gaps.std(ddof=1)); se=sd/np.sqrt(5)
    p=float(2*stats.t.sf(abs(mean/se),4)) if se else (0. if mean else 1.)
    width=float(stats.t.ppf(.975,4)*se)
    null=np.abs(np.asarray(list(itertools.product([-1,1],repeat=5)))@gaps/5)
    return dict(n=5,real_mean=float(real.mean()),rewired_mean=float(rewired.mean()),gaps=gaps.tolist(),
...
def preserve(message):
    ...
    flush_pending()
    paths=[ROOT,CFG,Path('TOPOLOGY_CONFIRMATION_PREREGISTRATION_V2.md'),Path('scripts/run_topology_confirmation_v2.py'),Path('tests/test_topology_confirmation_v2.py'),Path('data/raw/topology_confirmation_v2'),Path('results/compiler_v1/priority_handoff.json')]
...
def run():
    ...
    try:
        ...
        handoff=read('results/topology_confirmation_v1/superseded.json'); pid=handoff['retained_active_child'].get('pid')
        ...
        plan=freeze(ROOT/'frozen_plan.json',dict(pairs=pairs,inputs=hashes(sorted(set(map(str,inputs)))),test_gate='all_ten_checkpoints_complete'))
        preserve('Preregister topology confirmation: freeze five pairs and untouched test before training')
        checkpoints={}
        ...
        receipt=freeze(ROOT/'unlock.json',dict(split='test_confirm',choices_frozen=True,
            inputs=hashes([plan,*[p for pair in checkpoints.values() for p in pair.values()]]),all_ten_complete=True))
        outputs={}
        for seed,pair in pairs.items():
        ...
        lines=['# Topology confirmation v2 — five fresh pairs','',f"Primary criterion passed: **{summary['primary_pass']}**.",'',
        ...
        subprocess.run(['osascript','-e','display notification "Five paired seeds and locked final tests complete." with title "FlyGPT confirmation ready"'],check=False)
...
def register():
    ...
    inputs=[CFG,Path(__file__),Path('TOPOLOGY_CONFIRMATION_PREREGISTRATION_V2.md'),Path('tests/test_topology_confirmation_v2.py'),
