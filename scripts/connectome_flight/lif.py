...
(github.com/philshiu/Drosophila_brain_model, model.py, MIT licence) on the MaleCNS graph:
    ...
    stimulus: Poisson input on v with weight w_syn * f_poi; stimulated neurons have
...
class LifBrain:
    def __init__(self, graph_csr_pre, signs, stim_neurons, params=PARAMS, seed=1):
    ...
    def reset(self):
        ...
        self.buffer = np.zeros((self.dly_steps + 1, self.n), dtype=np.float64)
        self.t = 0
    ...
    def run(self, milliseconds, stim_rates_hz):
        ...
        self.t, self.state = _run(
            ...
            self.stim_neurons, np.asarray(stim_rates_hz, dtype=np.float64),
            counts, self.state,
...
def load_brain(stim_neurons, seed=1, params=None):
    ...
    return LifBrain(A, signs, stim_neurons, params=params or calibrated_params(), seed=seed)
